...
export interface CollaborationRealtimeServiceOptions {
  ...
  materializationRepository: CollaborationRealtimeMaterializationRepository | null;
...
const materializeEvent = async (
  ...
  switch (event.family) {
    case "personal_memory_changed":
      return materializePersonalMemoryChangedEvent(
        client.actor,
        event,
...
const authenticateRealtime = async (
  ...
  if (!context?.credential.operationFamilies.includes("team_workspace_read")) {
    throw Object.assign(
...
const createRealtimeReauthenticator = (
  ...
  return () => {
    ...
    return authenticateRealtime(freshRequest, auth);
  };
};
...
export const createCollaborationRealtimeService = async (
  ...
  const registerRoutes = () => {
    options.app.post("/v1/collaboration/realtime/snapshot", async (request) => {
      ...
      const realtimeAuth = await authenticateRealtime(request, options.auth);
    ...
    options.app.get(
      ...
      async (request, reply) => {
        ...
        const client: StreamClient = {
          ...
          reauthenticate: createRealtimeReauthenticator(request, options.auth),
