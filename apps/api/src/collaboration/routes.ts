...
const apiTokenForbidden = () =>
  Object.assign(
    new Error("API Tokens cannot authorize collaboration operations"),
    { statusCode: 403 }
  );
...
export const registerCollaborationRoutes = (
  ...
  app.get(
    ...
    async (request) => {
      const user = await authenticateCollaborationSession(request, context);
  ...
  const registerThreadMessageRoutes = (
    ...
    app.get(
      ...
      async (request) => {
        const user =
          scope === "personal"
            ? await authenticateCollaborationSession(request, context)
