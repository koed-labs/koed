...
import {
  bindConversationSourceGrantOperation,
  bindConversationSourceRevokeOperation,
  bindSharedMemoryFidelityChangeOperation,
...
const activationPathSchema = z
  .string()
  .regex(
    /^\/high-risk\/browser-activations\/[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
  );
...
export const highRiskActionGrantIntentSchema = z.discriminatedUnion("action", [
  ...
  z
    .object({
      ...
      candidateHash:
        createSharedMemoryCandidatePreviewSchema.shape.candidateHash,
      sourceRevision:
  ...
  z
    .object({
      action: z.literal("shared_memory.conversation_source_grant"),
      mutationId: uuidSchema,
      teamId: uuidSchema,
      shareGrantId: uuidSchema,
      expectedVersion:
        putTeamConversationSourceGrantSchema.shape.expectedVersion,
      mode: putTeamConversationSourceGrantSchema.shape.mode
    })
    .strict(),
  z
    .object({
      action: z.literal("shared_memory.conversation_source_revoke"),
      mutationId: uuidSchema,
      teamId: uuidSchema,
      shareGrantId: uuidSchema,
      expectedVersion:
        revokeTeamConversationSourceGrantSchema.shape.expectedVersion,
      reasonCode: revokeTeamConversationSourceGrantSchema.shape.reasonCode
    })
    .strict(),
  z
    .object({
  ...
  .superRefine((status, context) => {
    ...
    if (
      ...
      status.activationPath !==
        `/high-risk/browser-activations/${status.selector}`
    ) {
      context.addIssue({
        code: "custom",
        path: ["activationPath"],
...
export const resolveHighRiskActionGrantOperation = (input: {
  ...
  switch (intent.action) {
    ...
    case "shared_memory.conversation_source_revoke":
      return bindConversationSourceRevokeOperation(intent, clientRequestId);
    case "shared_memory.change_fidelity":
      return bindSharedMemoryFidelityChangeOperation(intent, clientRequestId);
