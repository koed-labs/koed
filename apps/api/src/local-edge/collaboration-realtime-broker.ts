...
import { z } from "zod";
import { materializePersonalMemoryChangedEvent } from "../collaboration/realtime.js";
import {
...
const remoteSubscriptionSchema = z
  .object({
    id: z.uuid(),
  ...
  .superRefine((value, context) => {
    ...
    if (
      ...
      context.addIssue({
        code: "custom",
        path: ["subscription"],
        message: "snapshot subscription is not active for the Team"
      });
    }
  });
...
type PendingEvent = PendingTeamEvent | PendingPersonalEvent;
...
export interface CollaborationRealtimeBrokerOptions {
  ...
  requireCollaborationMaterializationRepository: () => CollaborationRealtimeMaterializationRepository;
...
const publicSubscription = (row: LocalSubscriptionRow) => ({
  ...
  scope: { scope: "team" as const, teamId: row.teamId },
  state: row.state,
  version: row.version,
...
export const createCollaborationRealtimeBroker = (
  ...
  const postRemote = async (
    ...
    assertUpstreamOperationPathAllowed("team_workspace_read", "POST", path);
    let response: Response;
    try {
      response = await upstreamFetch(
  ...
  const persistSnapshot = async (
    ...
    withTransaction(requirePool(options.pool), async (client) => {
      const lockKey = `${pending.binding.upstreamBackendId}:${pending.binding.teamId}:${protocolVersion}`;
      await client.query(
      ...
      if (!existing) {
        const inserted = await client.query<LocalSubscriptionRow>(
          `insert into local_edge_collaboration_subscriptions (
             ...
             protocol_version, remote_subscription_id, remote_cursor,
             last_acknowledged_event_id, state, version, expires_at
           ) values ($1, $2, $3, $4, $5, $6, $7, null, 'requires_snapshot', 1, $8)
           returning ${selectColumns}`,
          [
            pending.localSubscriptionId,
            pending.binding.upstreamBackendId,
  ...
  const handleRemoteEvent = async (
    ...
    const pending: PendingTeamEvent = {
      kind: "team_event",
  ...
  const flushPersonalReplay = async (
    ...
    for (const event of replay.events) {
      ...
      const materialized = await materializePersonalMemoryChangedEvent(
        { userId: connection.binding.localOwnerUserId },
        event,
  ...
  const openRemoteStream = async (
    ...
    const query = new URLSearchParams({
      scope: "team",
      teamId: connection.binding.teamId,
  ...
  const registerRoutes = () => {
    ...
    options.app.post(
      ...
      async (request) => {
        ...
        const binding = await authorizeTeam(
          request,
        ...
        if (remote!.snapshot.teamId !== input.team_id) {
        ...
        return {
          ...
          subscription: {
            id: localSubscriptionId,
            protocolVersion,
            scope: { scope: "team", teamId: input.team_id },
            state: "awaiting_snapshot_ack",
    ...
    options.app.post(
      ...
      async (request) => {
        ...
        const pendingCandidate = pendingEvents.get(input.delivery_id);
        if (!pendingCandidate || pendingCandidate.kind !== "team_event") {
          if (input.event_id === null) {
        ...
        const eventPending = pendingCandidate as PendingTeamEvent;
        if (
