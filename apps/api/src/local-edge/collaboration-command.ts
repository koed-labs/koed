...
const desktopOperationFamily = (
  ...
  command.command === "collaboration.poll_action_grant" ||
...
const personalMessageFromRecord = (
  ...
  if (
    ...
    message.personalOwnerUserId !== user.id ||
    message.teamId !== null ||
    ...
    async (request, reply) => {
      ...
      if (!("upstream_backend_id" in input)) {
        ...
        if (command.command === "collaboration.select") {
          ...
          const view = await loadPersonalSelection({
      ...
      if (
        ...
        const preferredFamilies =
          input.command.command === "collaboration.request_action_grant"
            ...
            : (["action_grant", "share_grant_management"] as const);
        ...
        const result = await control.dispatch(input.command, {
          ...
          operationFamilies: new Set(
            [...context.operationFamilies].filter(
              (family): family is "action_grant" | "share_grant_management" =>
                family === "action_grant" || family === "share_grant_management"
            )
          ),
