...
export const collaborationActionGrantControlCommandNames = [
  ...
  "collaboration.poll_action_grant",
...
export interface CollaborationActionGrantIntentOperation extends HighRiskActionGrantOperation {
...
export interface CollaborationActionGrantControlContext {
  ...
  operationFamilies: ReadonlySet<"action_grant" | "share_grant_management">;
...
const createPollHint = (
  options: Required<
    Pick<
      CollaborationActionGrantControlOptions,
      "pollBaseMs" | "pollMaxMs" | "pollJitter" | "random"
    >
  >
): number => {
  const baseMs = Math.max(250, Math.floor(options.pollBaseMs));
  const maxMs = Math.max(baseMs, Math.floor(options.pollMaxMs));
  const jitter = Math.max(0, Math.min(options.pollJitter, 0.5));
  const random = Math.max(0, Math.min(options.random(), 1));
  const factor = 1 - jitter + random * jitter * 2;
  return Math.max(250, Math.min(maxMs, Math.round(baseMs * factor)));
};
...
const collaborationActionGrantOperationForIntent = (
  ...
  return operation === null || operation instanceof Promise
    ? null
    : {
...
): "action_grant" | "share_grant_management" =>
  operationFamily === "admin" ? "action_grant" : "share_grant_management";
...
export const createCollaborationActionGrantControl = (
  ...
  const options = {
    ...
    pollBaseMs: input.pollBaseMs ?? POLL_BASE_MS,
    pollMaxMs: input.pollMaxMs ?? POLL_MAX_MS,
    pollJitter: input.pollJitter ?? POLL_JITTER,
  ...
  const dispatch = async (
    ...
    try {
      switch (command.command) {
        case "collaboration.request_action_grant": {
          ...
          try {
            ...
            return success(
              command,
              status,
              status.state === "pending" ? createPollHint(options) : null
            );
          } catch (error) {
            updateCollaborationActionGrantCustodyStatus(
              options.koedHome,
        ...
        case "collaboration.poll_action_grant": {
          ...
          try {
            ...
            return success(
              command,
              next,
              next.state === "pending" ? createPollHint(options) : null
            );
          } catch (error) {
            persistAmbiguousWindow(
              options.koedHome,
