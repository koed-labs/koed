  ...
  async query<T = unknown>(sql: string, values: unknown[] = []) {
    ...
    if (
      ...
      const row: MemoryRow = {
        ...
        upstreamBackendId: values[1] as string,
        credentialBindingHash: values[2] as string,
        teamId: values[3] as string,
        protocolVersion: values[4] as number,
        remoteSubscriptionId: values[5] as string,
        remoteCursor: values[6] as string,
    ...
    if (
      normalized.startsWith("update local_edge_collaboration_subscriptions") &&
      normalized.includes("credential_binding_hash <> $4")
    ) {
      const rows = this.rows.filter(
        (candidate) =>
          candidate.upstreamBackendId === values[0] &&
          candidate.teamId === values[1] &&
          candidate.protocolVersion === values[2] &&
          candidate.credentialBindingHash !== values[3] &&
...
const createHarness = async (
  ...
  const remoteFetch: typeof fetch = async (input, init) => {
    ...
    if (pathname.endsWith("/snapshot")) {
      return Response.json(snapshotResponse(String(body?.teamId)));
    }
