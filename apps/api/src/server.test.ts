...
describe("account and access flows", () => {
  ...
  it("binds PDS enrollment challenges to browser auth and the requesting device deployment", async () => {
    ...
    const registered = await app.inject({
      ...
      url: "/auth/register",
      payload: {
        email: `pds-challenge-${randomUUID()}@example.com`,
        password: "password123"
      }
    });
    const response = await app.inject({
      ...
      headers: { cookie: cookieHeader(registered) },
      payload: { device_deployment_id: "deployment-new-device" }
    });
    expect(response.statusCode).toBe(200);
    expect(challenges).toHaveLength(1);
    expect(challenges[0]).toMatchObject({
      browserDeploymentId: "deployment-new-device",
      groupId: undefined
    });
  ...
  it("rejects an Authority-only PDS membership transition", async () => {
    const authority = generateKeyPairSync("ed25519");
    const authorityPublic = authority.publicKey.export({
      format: "jwk"
    }) as JsonWebKey;
    const member = generateKeyPairSync("ed25519");
    const memberPublic = member.publicKey.export({
      format: "jwk"
    }) as JsonWebKey;
    const repository = createFakeRepository();
    Object.assign(repository, {
      getPersonalDeviceGroup: async () => ({
        id: randomUUID(),
        groupId: "group-authority-only",
        authorityKeyId: "authority-key",
        authorityPublicKey: authorityPublic.x!,
        recoverySigningKeyId: "recovery-signing",
        recoverySigningPublicKey: randomBytes(32).toString("base64url"),
        recoveryKemKeyId: "recovery-kem",
        recoveryKemPublicKey: randomBytes(32).toString("base64url"),
        currentEpoch: "1",
        headSequence: "1",
        headHash: randomBytes(32).toString("base64url"),
        state: "active",
        stateReason: null,
        members: [
          {
            deviceId: "device-member",
            signingKeyId: "member-signing",
            signingPublicKey: memberPublic.x!,
            kemKeyId: "member-kem",
            kemPublicKey: randomBytes(32).toString("base64url"),
            operationFamilies: ["pds_relay"],
            status: "active",
            admittedSequence: "1",
            revokedSequence: null,
    ...
    const registered = await app.inject({
      ...
      url: "/auth/register",
      payload: {
        email: `pds-authority-${randomUUID()}@example.com`,
        password: "password123"
      }
    });
    const group = await (
      repository as typeof repository & {
        getPersonalDeviceGroup: () => Promise<{
          headHash: string;
        }>;
      }
    ).getPersonalDeviceGroup();
    const draft = {
      protocol: "koed/pds/v1",
      kind: "revoke-device",
      groupId: "group-authority-only",
      sequence: "2",
      previousHash: group.headHash,
      body: {
        deviceId: "device-member",
        reasonCode: "user_requested",
        revokedAt: "2026-07-24T00:00:00.000Z",
        previousEpoch: "1",
        nextEpoch: "2",
        keyBundleHash: randomBytes(32).toString("base64url")
      }
    };
    const response = await app.inject({
      method: "POST",
      url: "/v1/personal-device-sync/groups/group-authority-only/transitions",
      headers: { cookie: cookieHeader(registered) },
      payload: {
        statement: canonicalizePdsJson({
          draft,
          authorization: {
            signerKeyId: "member-signing",
            signature: signPdsGroupDraft(draft, authority.privateKey)
          }
        })
      }
    });
