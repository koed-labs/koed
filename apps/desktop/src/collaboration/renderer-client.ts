  ...
  const waitForActionGrant = async (
    ...
    while (status.state === "pending") {
      ...
      await new Promise<void>((resolve) =>
        setTimeout(resolve, Math.min(status.pollAfterMs ?? 1_500, remainingMs))
      );
      if (disposed) throw new CollaborationClientError(offlineError());
      if (grantAuthorityGeneration !== authorityGeneration) {
        throw new CollaborationClientError(accessRevokedError());
      ...
      try {
        result = await command("collaboration.poll_action_grant", {
        ...
        await new Promise<void>((resolve) =>
          setTimeout(
            resolve,
            Math.min(
              Math.max(caught.retryAfterMs ?? status.pollAfterMs ?? 1_500, 250),
              retryRemainingMs
            )
          )
        );
      ...
      if (!result.ok || result.command !== "collaboration.poll_action_grant") {
        throw new Error("Unexpected Action Grant poll result.");
      }
