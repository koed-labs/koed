...
const validateClosedSession = (value: unknown): PdsClosedSessionMetadata => {
  ...
  if (session.closed !== true || session.captureMethod !== "transcript") {
...
const packageRecipients = (
  ...
  if (
    new Set(sorted.map((recipient) => recipient.kemKeyId)).size !==
    sorted.length
  ) {
    throw new TypeError("PDS recipient KEM keys must be unique");
  }
