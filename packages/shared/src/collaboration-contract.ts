...
export const collaborationActionGrantStatusSchema = z
  .object({
    ...
    pollAfterMs: z.number().int().min(250).max(30_000).nullable()
  })
  .strict()
  .superRefine((status, context) => {
    ...
    if (pending !== (status.pollAfterMs !== null)) {
      context.addIssue({
        code: "custom",
        path: ["pollAfterMs"],
        message: pending
          ? "Pending Action Grants require a bounded poll hint"
          : "Terminal or approved Action Grants must not include a poll hint"
      });
    }
  });
...
export const collaborationRendererCommandSchema = z
  .discriminatedUnion("command", [
    ...
    command("collaboration.poll_action_grant", {
      actionGrant: collaborationActionGrantReferenceSchema
    }),
