...
The resulting MCP block contains the executable, the owner-scoped runtime
location, and a tool-level approval rule for read-only recall:
...
The rule lets a managed Conversation use Koed recall without a separate tool
approval. It does not pre-approve Curated Memory intake or other write-capable
tools; those follow the selected Conversation permission mode.
