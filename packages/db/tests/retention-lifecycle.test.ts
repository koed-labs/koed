...
describeDb("retention lifecycle repository", () => {
  ...
  const createSharedMemoryFixture = async (
    ...
    const personalWorkspace = await pool.query<{ id: string }>(
      `insert into workspaces (owner_user_id, visibility, name)
       values ($1, 'personal', $2) returning id`,
      [fixture.userId, `Retention Personal ${randomUUID()}`]
    );
    const session = await pool.query<{ id: string }>(
      `insert into sessions (
         owner_user_id, workspace_id, visibility, source_runtime, capture_method
       ) values ($1, $2, 'personal', 'codex', 'hook') returning id`,
      [fixture.userId, personalWorkspace.rows[0]!.id]
    );
  ...
  const createOwnerReplicaFixture = async (
    ...
    const session = await pool.query<{ id: string }>(
      `insert into sessions (
         owner_user_id, workspace_id, visibility, source_runtime,
         capture_method, external_session_id, metadata
       ) values ($1, $2, 'personal', 'codex', 'api', $3, $4::jsonb)
       returning id`,
      [
        fixture.userId,
        personalWorkspace.rows[0]!.id,
        `remote-${randomUUID()}`,
