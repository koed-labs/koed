...
CREATE INDEX "team_conversation_source_grants_logical_source_active_idx" ON "team_conversation_source_grants" USING btree ("logical_source_id","updated_at" DESC NULLS LAST) WHERE "team_conversation_source_grants"."lifecycle" = 'active';--> statement-breakpoint
ALTER TABLE "team_session_share_grants" ADD CONSTRAINT "team_session_share_grants_source_scope_unique" UNIQUE("id","team_id","team_workspace_id");