...
const notifyPdsRelayWake = (client: pg.PoolClient, groupId: string) =>
...
export const createPersonalDeviceSyncLifecycleRepository = (pool: pg.Pool) => ({
  ...
  async getPdsLifecycleControl(input: {
    ...
    const controls = await pool.query<{
      ...
      `select lifecycle.sequence,lifecycle.kind,lifecycle.canonical_record,lifecycle.canonical_statement
       from (
       ...
       order by lifecycle.sequence::numeric limit $3`,
  ...
  async reconcilePdsRestore(input: {
    ...
    try {
      ...
      const prior = await client.query<{
        ...
        `select authority_head,authority_sequence,lifecycle_high_water,restore_high_water from pds_replica_lifecycle_state s
