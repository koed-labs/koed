    ...
    `select id,owner_user_id,session_id,metadata,raw_json,raw_text
      ...
      where visibility='personal'
        ...
        and (
          ...
          or metadata ->> 'approvalReview' = 'true'
          or metadata ? 'approvalKind'
          or metadata ? 'providerApprovalKind'
          or coalesce(metadata ->> 'transcriptType','') like 'approval\\_%' escape '\\'
        )
     ...
     select
       ...
       (select count(distinct share_grant.id)::int
         ...
         where memory.local_session_id=event.session_id
           ...
           and event.source_sequence <= consent.source_revision) as snapshot_shares,
       (select count(distinct share_grant.id)::int
          from team_session_share_grants share_grant
          join source_owner_representation_consents consent on consent.id=share_grant.consent_id
          join logical_memories memory on memory.id=share_grant.logical_memory_id
         ...
         where memory.local_session_id=event.session_id
           and consent.mode='snapshot'
           and share_grant.lifecycle='active'
           and event.source_sequence is null) as ambiguous_snapshot_shares,
    ...
    const excluded = itemIds.length
      ? await client.query(
          `update conversation_items
            ...
            where id=any($1::uuid[])
              and (memory_excluded_at is null or memory_exclusion_reason<>'approval_activity_policy_v1')`,
          [itemIds]
        )
      : { rowCount: 0 };
      ...
      ? await client.query<{ id: string }>(
          `select distinct share_grant.id
            ...
            where memory.local_session_id=event.session_id
              and consent.mode='snapshot'
              and share_grant.lifecycle='active'
              and event.source_sequence is null
              ...
              and exists (
                select 1
                 ...
                 where event.id=any($1::uuid[])
                   ...
                   and event.source_sequence is not null
                   and event.source_sequence <= consent.source_revision
              )
