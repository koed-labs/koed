...
export const createPersonalDeviceSyncLocalRepository = (
  ...
  async claimPdsOutbox(input) {
    ...
    const rows = await pool.query<PdsClaimedOutboxEntry>(
      `with claimed as (
         ...
         order by o.retry_at,o.id for update skip locked limit $1
       ...
       where o.id=c.id returning o.id,o.closure_id as "closureId",s.package_id as "packageId",s.source_manifest_hash as "sourceManifestHash",o.attempt_count as "attemptCount"`,
  ...
  async retryPdsOutbox(input) {
    const result = await pool.query(
      `update pds_outbox_entries set state='pending',last_error_class=$3,retry_at=$4,lease_owner=null,lease_until=null,updated_at=now()
  ...
  async claimPdsInbox(input) {
    ...
    const rows = await pool.query<PdsClaimedInboxEntry>(
      `with claimed as (
         select id from pds_inbox_entries where state in ('pending','downloading','verifying','processing')
  ...
  async markPdsInboxFailure(input) {
    const result = await pool.query(
      `update pds_inbox_entries set state='failed',last_error_class=$3,retry_at=$4,lease_owner=null,lease_until=null,updated_at=now()
       where id=$1 and lease_owner=$2`,
      [
        input.inboxId,
        input.workerId,
  ...
  async setPdsPublicationPaused(input) {
    const result = await pool.query(
      `update pds_outbox_entries o set state=case when $3 then 'paused' else 'pending' end,
       ...
       from pds_session_closures c join personal_device_groups g on g.id=c.group_id
       join local_personal_identities i on i.id=g.local_personal_identity_id
       where o.closure_id=c.id and i.owner_user_id=$1 and g.group_id=$2
  ...
  async getPdsLocalSyncStatus(input) {
    const group = await pool.query<{ id: string; enabled: boolean }>(
      `select g.id,p.enabled from personal_device_groups g
       join local_personal_identities i on i.id=g.local_personal_identity_id
       join personal_sync_policies p on p.group_id=g.id
       where i.owner_user_id=$1 and g.group_id=$2`,
      [input.userId, input.groupId]
    ...
    const [outbox, inbox, replicas, heartbeat] = await Promise.all([
      pool.query<{ state: string; count: string }>(
        "select state,count(*)::text from pds_outbox_entries group by state"
      ),
      pool.query<{ state: string; count: string }>(
    ...
    return {
      enabled: row.enabled,
      paused:
        (outboxCounts.paused ?? 0) > 0 &&
        (outboxCounts.pending ?? 0) +
          (outboxCounts.uploading ?? 0) +
          (outboxCounts.committed ?? 0) ===
          0,
