...
create table if not exists memory_questions (
  ...
  check (retrieval_scope in ('personal', 'personal+team')),
  check (
...
create index if not exists memory_questions_team_created_idx
  on memory_questions(team_id, created_at desc, id desc)
  where visibility = 'team';

create index if not exists memory_questions_team_scope_idx
  on memory_questions(team_id, search_domain, workspace_id, session_id, created_at desc, id desc)
  where visibility = 'team';
