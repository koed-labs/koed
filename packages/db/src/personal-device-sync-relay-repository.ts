...
const transportRecord = (
  ...
  const rawPendingRecipientCount = value.pending_recipient_count;
  const pendingRecipientCount =
    typeof rawPendingRecipientCount === "string" ||
    typeof rawPendingRecipientCount === "number" ||
    typeof rawPendingRecipientCount === "bigint"
      ? String(rawPendingRecipientCount)
      : "0";
  return {
    ...
    deliveryState:
      pendingRecipientCount === "0" && value.state === "committed"
        ? "acked"
        : "pending",
...
const getAccessibleTransport = async (
  client: pg.PoolClient,
  input: PdsRelayAuthContext,
  transportId: string,
  lock = false
  ...
  const result = await client.query(
    `select t.*,
       (select count(*)::text from pds_relay_recipients pending
        where pending.transport_id=t.id and pending.acked_at is null and pending.waived_at is null)
         as pending_recipient_count
     from pds_relay_transports t
     where t.group_id=$1 and t.transport_id=$2 and t.state='committed' and t.expires_at>now()
       and (
         t.sender_device_id=$3 or exists (
           select 1 from pds_relay_recipients recipient
           where recipient.transport_id=t.id and recipient.recipient_device_id=$3
         )
...
export const pdsRedactedRelayReceipt = (input: {
  groupId: string;
  transportId: string;
  packageId: string;
  sourceManifestHash: string;
...
export const pdsRelayDeliveryRecipients = (
  ...
  intendedRecipientSnapshot.filter(
    (recipientDeviceId) => recipientDeviceId !== servingDeviceId
  );

const pendingRelayRecipients = async (
  client: pg.PoolClient,
  transportId: string
): Promise<string> => {
  const result = await client.query<{ count: string }>(
    `select count(*)::text as count from pds_relay_recipients
...
export const createPersonalDeviceSyncRelayRepository = (pool: pg.Pool) => ({
  ...
  async initializePdsRelayTransport(
    ...
    try {
      ...
      const deliveryRecipients = pdsRelayDeliveryRecipients(
        header.intendedRecipientSnapshot,
        header.servingDeviceId
      );
  ...
  async getPdsRelayTransportMetadata(
    ...
    try {
      await client.query("begin");
      const auth = await assertCurrentRelayAuth(client, input);
      const transport = await getAccessibleTransport(
        client,
        auth,
        input.transportId
      );
  ...
  async hasPdsRelayAcknowledgement(
    input: PdsRelayAuthContext & { transportIds: string[] }
  ): Promise<boolean> {
    if (!input.transportIds.length) return false;
    const client = await pool.connect();
    try {
      await client.query("begin");
      const auth = await assertCurrentRelayAuth(client, input);
      const result = await client.query(
        `select exists (
           select 1 from pds_relay_transports t
           where t.group_id=$1 and t.sender_device_id=$2
             and t.transport_id=any($3::text[]) and t.state='committed'
             and not exists (
               select 1 from pds_relay_recipients r
               where r.transport_id=t.id and r.acked_at is null and r.waived_at is null
             )
         ) as acknowledged`,
        [auth.groupDbId, auth.deviceId, input.transportIds]
      );
      await client.query("commit");
      return row<{ acknowledged: boolean }>(result.rows[0]).acknowledged;
    } catch (error) {
      await client.query("rollback");
      throw error;
    } finally {
    ...
    try {
      ...
      if (transport.state === "committed") {
        if (transport.package_digest !== input.packageDigest)
          throw securityError();
        const pendingRecipientCount = await pendingRelayRecipients(
          client,
          transport.id as string
        );
        await client.query("commit");
        return transportRecord({
          ...transport,
          pending_recipient_count: pendingRecipientCount,
          missing_chunks: []
        });
      }
      const missing = await missingChunks(
      ...
      await client.query(`select pg_notify('koed_pds_relay_wake', $1)`, [
        auth.groupId
      ]);
      const pendingRecipientCount = await pendingRelayRecipients(
        client,
  ...
  async acknowledgePdsRelayPackage(
    ...
    try {
      ...
      if (row<{ count: number }>(pending.rows[0]).count === 0) {
        ...
        await client.query(
          `update pds_outbox_entries set state='acked',updated_at=now()
           where transport_id=$1 and state='committed'`,
          [transportId]
        );
      }
      await client.query(`select pg_notify('koed_pds_relay_wake', $1)`, [
      ...
      await client.query(
        `update pds_tombstone_acks a set waived_at=now(),waiver_statement_hash=s.statement_hash
         ...
         where a.tombstone_id=t.id and a.acked_at is null and a.waived_at is null
           and m.device_id=a.device_id and a.device_id=any(t.active_device_snapshot) and m.status='revoked'
