...
```bash
docker compose -f examples/docker-compose/docker-compose.yml exec postgres pg_dump -U koed koed > koed-backup.sql
```

Example restore into a stopped/fresh stack:

```bash
docker compose -f examples/docker-compose/docker-compose.yml exec -T postgres psql -U koed koed < koed-backup.sql
```
