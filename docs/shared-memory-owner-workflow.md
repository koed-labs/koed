...
| Activate Pending Share            | Complete the accepted share asynchronously             | Start sync only after acceptance; require the exact source revision; create authoritative preview and consent; materialize the representation; resolve the deterministic companion before marking the Pending Share activated         | Worker restart or local interruption reuses durable identities; failures remain owner-visible; activated legacy rows missing a companion are repaired idempotently |
