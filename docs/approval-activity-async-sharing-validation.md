...
- API routes, realtime, high-risk binding, local-edge control, and command
  registry: 160 focused tests.
- The complete Desktop suite passes 423 tests with one skipped platform case.
  It includes the active `Personal > Memory > Shares` route rather than only
  the legacy collaboration route.
- The complete Shared Memory repository suite passes 46 real-Postgres tests.
...
- The required Postgres-backed collaboration suites pass against a separate
  ...
  tests.
- Shared, DB, API, Desktop, and MCP Server type checks pass.
...
| Stage                    |      Time |
| ------------------------ | --------: |
| Candidate preview        |  2.823 ms |
| Authoritative preview    | 12.815 ms |
| Pending Share acceptance |  4.738 ms |
| Pending Share activation | 35.425 ms |
