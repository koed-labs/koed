...
5. Reconcile encryption, lifecycle, authorization, sync high-water state, and accepted Curated Memory evidence manifests before any derived publish. If event-only or node-only evidence lacks a provable stable canonical anchor/rebuild mapping, abort `preserve_canonical`; do not invalidate its source or mark its assertion suppressed.
...
| PDS and Directed Hosted Sync       | Source/package lineage survives; local derivation readiness gates publication; PDS peers need not match local processing identity; frozen PDS V1 signed manifests/package bytes contain no processing metadata.                                                                              |
