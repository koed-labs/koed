...
3. Verify installed release assets and retained old/new prompt bundles, models/settings boundaries, tokenizers, and implementation manifests needed for candidate build and rollback.
4. Create encrypted Postgres and required non-derivable local-state backups. Verify archive integrity and key access.
5. Export redacted pre-cutover status: canonical counts/hashes by scope, lifecycle floors, sync cursors, current generation-set revision, coverage state, queue state, and backup proof.
...
- PDS governance state from KOE-348 / PR #319 only after that work merges and is revalidated against `main`.
...
- embeddings, vector partitions, auxiliary indexes, graph-derived state, caches, and readiness aggregates;
...
3. Apply database revision; seed clean release-V1 payload baselines and generation-aware registry support; invalidate alpha capability caches.
4. Import preserved canonical records or create a clean database.
5. Reconcile encryption, lifecycle, authorization, and sync high-water state before any derived publish.
...
- AI Client outage leaves candidate LCM work pending. There is no backend LLM fallback.
...
| Canonical-preserving upgrade       | Canonical counts, identities, ownership, lifecycle, encryption, and sync cursors match pre-cutover; derived IDs may change; Recall stays on a complete published generation. |
...
| Local/remote capability            | Fresh compatible source/payload/protocol support is required; stale/unknown capability state fails before intake or Recall.                                                  |
| PDS and Directed Hosted Sync       | Source/package lineage survives; local derivation readiness gates publication; PDS peers need not match local processing identity.                                           |
