...
5. Export redacted pre-cutover status: canonical counts/hashes by scope, lifecycle floors, sync cursors, current generation-set revision, capability-schema-6 collaboration/realtime status, coverage state, queue state, and backup proof.
...
3. Apply database revision; seed clean release-V1 payload baselines and generation-aware registry support; invalidate capability caches for current alpha schemas `2`-`6`.
...
5. Reconcile encryption, lifecycle, authorization, sync high-water state, accepted Curated Memory evidence manifests, and retained PDS artifact disposition before any derived publish. If event-only or node-only evidence lacks a provable stable canonical anchor/rebuild mapping, abort `preserve_canonical`; do not invalidate its source or mark its assertion suppressed.
