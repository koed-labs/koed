...
Cross-Identity Sync umbrella specializations. PDS V1 now exposes
browser-session Personal Device Group authority routes only: challenge,
genesis, Key Bundle countersigning, membership transition, scoped
group/status/Key Bundle/certificate retrieval, policy, and opaque Remote Account
...
PDS protocol is
[Personal Device Sync Protocol V1](personal-device-sync-protocol.md). Any future
eligible closed Captured Session sequence remains separate: source seals
contiguous raw closure; origin signs JCS source manifest; source encrypts package
and recipient CEK envelopes; relay stores encrypted chunks; every active device
verifies membership/log, signatures, hashes, and AEAD before local
