...
- `MEMORY_MANUAL_ANSWER_MODEL_OPTIONS`: comma-separated Codex model options shown in the Explorer manual Memory Question selector.
...
Manual Memory Question settings selected in the Explorer are stored on the question row so retry and background catch-up use the same model, reasoning effort, timeout, and attempts. If Codex app-server cannot be started, local Synthesis fails visibly instead of falling back to a backend LLM path.
