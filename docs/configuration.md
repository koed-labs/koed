...
raw 32-byte Ed25519 material. Keep the seed only in the deployment secret
provider; never put it in `server.json`, `KOED_HOME`, browser config, logs,
database, or responses. Missing or malformed signer configuration reports PDS
capability unavailable and all PDS governance routes fail closed. No local/test
signer fallback exists in managed deployments.

This control plane conforms to
[Personal Device Sync Protocol V1](personal-device-sync-protocol.md) and stores
only public keys, committed signed statements, encrypted recipient envelopes,
and redacted audit metadata. An authorized client first requests an Authority
countersignature over its Key Bundle, then signs the membership statement that
binds the finalized bundle hash; the transition transaction revalidates and
commits both together. The preparation request changes no group state. It does
not implement relay, package transport, materialization, or Desktop flows. API Tokens, device credentials,
Cross-Identity Sync configuration, canonical JSON helpers, and RSA envelope
settings do not authorize or enable PDS governance. Device signing/KEM,
recovery, Authority, epoch, source-fingerprint, tombstone, and Project keys
remain separate; ordinary `KOED_HOME` configuration must never contain PDS
