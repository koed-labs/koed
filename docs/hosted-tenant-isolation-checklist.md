...
| Share Grant source boundary             | Team reads must use active Share Grants for the same Team and Workspace. Revoked grants and other Workspaces must be excluded.             | `packages/db/src/repository.ts`; `packages/db/src/schema.ts`; `packages/db/tests/repository.test.ts`.                                                                                               | Covered with automated tests.                                                                                                                                    |
...
| Logging and diagnostics                 | Logs and diagnostics must redact secrets and raw memory.                                                                                   | `apps/api/src/server/logging.test.ts`; `apps/worker/src/logging.test.ts`; `packages/mcp-server/tests/logger.test.ts`; `docs/security.md`.                                                           | Covered for current server/worker logging surfaces; revisit for hosted control-plane services.                                                                   |
...
Current automated coverage exists for the first four groups in the API, core,
and database test suites. Billing gates are tracked by KOE-191. Support/admin
operations are tracked by KOE-199. Hosted observability is tracked by KOE-194.
