...
Browser-session PDS routes provide genesis, enrollment challenge, Authority
countersigning of an active-device- or recovery-authorized Key Bundle, signed
group transitions, scoped status/log/certificate/bundle retrieval, Personal
Sync Policy, and Remote Account Link records. Clients first obtain the finalized
Key Bundle and its hash, then sign the membership statement that binds that
exact hash; Authority verifies and commits both in one transition transaction.
Each challenge and proof binds the browser-authenticated local subject to the
requesting device's signed deployment ID; the Authority server's own deployment
identity is never substituted for the device identity. Bearer API Tokens and
`Koed-Device` credentials are denied. Stale valid
transitions return the current signed head without rebasing or freezing;
governance freezes only on verified Authority equivocation or durable integrity
evidence. Remote Account Link accepts an opaque proof token only and fails
closed without a server verifier. Tombstone and conflict resolution remain
unavailable pending exact lifecycle persistence.

The control plane is separate from Directed Hosted Cross-Identity Sync and its
RSA envelopes. PDS remains current `koed/pds/v1` only and follows
[Personal Device Sync Protocol V1](personal-device-sync-protocol.md). Relay and
package delivery, materialization, and Desktop flows remain unimplemented;
policy defaults off and covers only future closed Captured Sessions. Local
capture and Recall continue during Authority outage or equivocation freeze. A
perfect same-path full-machine clone remains undetectable locally; remote
collision detection and explicit re-enrollment remain required.
