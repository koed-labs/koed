...
The inventory and discussion below preserve the design rationale. Historical
...
Before tiered approval, Koed sent every high-risk Desktop collaboration action
through the same approval sequence:
...
- the old Shared Memory workflow required separate approval for preview,
  consent, and the resulting Share Grant;
...
This table records the implemented approval tiers for the Desktop actions.
...
| Shared Memory            | Prepare a source preview                       | `collaboration.preview_shared_memory`         | `shared_memory.preview`         | Browser approval | **Direct**         | Read-like preparation for the source owner. It creates no active source-owner policy, consent, or Share Grant.                                                                                                                                                           |
| Shared Memory            | Share Memory with a Workspace                  | `collaboration.share_memory`                  | `shared_memory.pending_share`   | Tiered approval  | **Risk-sensitive** | One exact review binds the typed source, destination, mode, fidelity, activation representation, Curated Memory choice, preview, and expiry. Acceptance creates one durable Pending Share. Memory Events use **Step-up**. Derived representations use **Native review**. |
...
| Shared Memory            | Change Shared Memory fidelity                  | `collaboration.change_shared_memory_fidelity` | `shared_memory.change_fidelity` | Tiered approval  | **Risk-sensitive** | One exact review binds a replacement preview and consent. Increasing fidelity or enabling Curated Memory uses **Step-up**. Reducing fidelity uses **Native review**. The change creates a Pending Share.                                                                 |
...
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First-time Shared Memory share                               |                                                               One preview plus one reviewed Pending Share acceptance | Preview directly. Then use one review for consent and durable acceptance. Memory Events use Step-up at this final decision.                                            |
| Change Shared Memory fidelity                                |                                                   One replacement preview plus one reviewed Pending Share acceptance | Preview directly. Then use one review. Increasing fidelity uses Step-up.                                                                                               |
