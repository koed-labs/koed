...
| Shared Memory            | Share Memory with a Workspace                  | `collaboration.share_memory`                        | `shared_memory.share`                 | Browser approval | **Risk-sensitive** | One exact review shows source, Team, Workspace, representation, mode, and expiry, then atomically activates any proposed owner policy and creates consent plus the Share Grant. Raw Memory Events use **Step-up**; derived representations use **Native review**. |
...
| ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First-time Shared Memory share                               |                                                                                           3: preview, consent, share | Preview directly, then one exact review for policy activation, consent, and Share Grant. Raw Memory Events use Step-up at this final decision only.                    |
| Change Shared Memory representation                          |                                                                           3: preview, consent, representation change | Preview directly, then one exact review; require Step-up only when the change increases detail.                                                                        |
