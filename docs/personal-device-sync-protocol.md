...
record `ordinal`; metadata permits only `contentType`, `sourceRole`, `toolName`,
and `toolCallId`, each a bounded string. This is an immutable source-item
profile, not a Projection/event schema. Paths, credentials, Team fields,
