...
directed Cross-Identity Sync uses a distinct RSA-OAEP target-envelope contract
and must not be reused as PDS V1.
...
```text
UTF8("koed/pds/v1/" + recordType + "\n") || UTF8(JCS(recordWithoutSignature))
```
...
other type. Signature objects contain `signerKeyId` and `signature`; neither is
included in bytes signed by that signer. Countersignatures use same formula with
`recordType` `group-statement` or `tombstone` and authority key id.
...
(base64url SHA-256). `rawClosure` counts/hash and `sourceClosureHash` cover JCS
of ordered records. `originSignature` is excluded only from source-manifest
signature bytes as defined in section 2.
...
CSPRNG and used once. Payload AAD is JCS UTF-8 transport header excluding ciphertext/tag and
signatures. It includes exactly protocol, group id, package id, source manifest
hash, origin device id, `contentEpoch`, total plaintext bytes, chunk count, and
payload nonce. `contentEpoch` is immutable source-package metadata. Nonce reuse
under a CEK is fatal and package is rejected. Compression is forbidden before or
after encryption.
...
`chunkCount`, `payloadNonce`, `payloadCiphertextHash`, `expiresAt`, and
`originTransportSignature` (`signerKeyId`, `signature`). Signature bytes omit
only `originTransportSignature`; `payloadNonce` is 12-byte base64url and all
hashes are base64url SHA-256. Each chunk has exactly `protocol`, `groupId`,
