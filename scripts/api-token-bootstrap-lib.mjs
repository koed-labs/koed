...
export const formatCliError = (error) => {
  ...
  if (code && databaseConnectionCodes.has(code)) {
    return [
      ...
      "Start the Operator-managed Postgres dependency (for example `docker compose -f examples/docker-compose/docker-compose.yml up postgres`) or update DATABASE_URL in `.env`.",
